...
listOfInstances=""
messages="Following Instances have port XX open"
...
try:
    for securityGroup in sg:
        for rule in securityGroup.rules:
            global instanceId;
            if (rule.from_port=='XX' and rule.to_port == 'XX') and '0.0.0.0/0' in str(rule.grants):
                for instanceid in securityGroup.instances():
                    ...
                    connSNS.publish(topic='SNS-topic-arn-endpoint',message = messages + "\n" + listOfInstances, subject='ProjectName : Server List with Port X Open')
